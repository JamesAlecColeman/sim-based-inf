...
def ids_to_storage_iter_nos(pop_ids):
...
def get_best_x_rts_or_ats(run_dir, iter_no, x_best_indices, repol=True):
    ...
    iter_nos_to_pop_ids = ids_to_storage_iter_nos(best_x_ids)
...
inferences_folder, repol, save_analysis = "Inferences_qrs_local", 0, 1
compare_to_truth = 1
patient_id_select = None # Set to None to just find all targets
run_id_select = None
force_iter_final = "max" #"max"
patient_id_skip = None
correct_targ_leads_len = 0
benchmarks_folder = "New_Benchmarks_APDs"
coarse_dx = 2000
stop_thresh = 0.00002
iter_step, x_best = 50, 10  # Analyse convergence every iter_step iterations, just the x_best solutions
separate_scaling = 1

inferences_path = f"{main_dir}/{inferences_folder}"
targets_in_inf_folder, runs_in_targets = find_inference_runs(inferences_path)
...
for i_targ, target in enumerate(runs_in_targets.keys()):  # E.g. now in "DTI003_500_ctrl"
    ...
    print(f"{target=}")
    #target = "DTI004_500_ctrl"  # override
    ...
    if compare_to_truth:
        # TODO only load this where this is known
    ...
    for i_run, run_id in enumerate(runs_in_targets[target]):  # E.g. now in ""DTI003_500_ctrl/runtime_512_-10.0""
        ...
        print(f"{min_diff_score=}")
        print(f"{min_reg_score=}")

        print(f"Stopped at iteration {i_iter_final} of {n_iterations} iterations")
        # Extract best model at final iteration
        best_x_times, best_x_reg_scores, best_x_leads = get_best_x_rts_or_ats(run_path, i_iter_final, 1,
                                                                              repol=repol)
        ...
        for iter_no in range(0, i_iter_maximum, iter_step):
            # TODO double check reg scores vs. diff scores
            best_x_times, best_x_reg_scores, best_x_leads = get_best_x_rts_or_ats(run_path, iter_no, x_best,
                                                                                  repol=repol)
            ...
            # TODO if repol calc APDs as well
            # TODO must ensure u use correct activation times from mother dir dependent on run_id
        ...
        # TODO surely scores can be replaced with the stopping condition results?
        ...
        # TODO identify if this is still necessary for repolarisation as well as activation
        ...
        # TODO must ensure reg is correct for repols
        # TODO identify if this length change is necessary for repolarisation and why it occurs sometimes
        if correct_targ_leads_len:
            target_leads_normed = {name: target_leads_normed[name][:len(sim_leads_normed["I"])] for name in lead_names}
            leads_target = {name: leads_target[name][:len(leads_sim_best["I"])] for name in lead_names}
        ...
        leads_target = {name: leads_target[name][target_qrs_idxs] for name in lead_names}
        times_target_s = times_s  # Because match_sim_and_target_times is matching targ times to sim times

        if not separate_scaling:
            # One global overall scaling
            # Optimal global scaling to allow us to compare the 2 leads
            alpha = qrsm.find_optimal_scaling(leads_sim_best, leads_target)
            leads_sim_rescaled = {name: leads_sim_best[name] * alpha for name in lead_names}
        else:

            # Separate precordial vs. limb scaling
            lead_names = list(leads_sim_best.keys())

            # Define lead groups
            limb_leads = ['I', 'II', 'III', 'aVR', 'aVL', 'aVF']
            prec_leads = ['V1', 'V2', 'V3', 'V4', 'V5', 'V6']

            # Filter only present leads in each group
            limb_present = [lead for lead in limb_leads if lead in leads_sim_best and lead in leads_target]
            prec_present = [lead for lead in prec_leads if lead in leads_sim_best and lead in leads_target]

            # Build dicts for scaling
            sim_limb = {lead: leads_sim_best[lead] for lead in limb_present}
            targ_limb = {lead: leads_target[lead] for lead in limb_present}

            sim_prec = {lead: leads_sim_best[lead] for lead in prec_present}
            targ_prec = {lead: leads_target[lead] for lead in prec_present}

            # Compute separate scaling factors
            alpha_limb = qrsm.find_optimal_scaling(sim_limb, targ_limb) if limb_present else 1.0
            alpha_prec = qrsm.find_optimal_scaling(sim_prec, targ_prec) if prec_present else 1.0

            # Apply scaling
            leads_sim_rescaled = {}
            for lead in lead_names:
                if lead in limb_present:
                    leads_sim_rescaled[lead] = leads_sim_best[lead] * alpha_limb
                elif lead in prec_present:
                    leads_sim_rescaled[lead] = leads_sim_best[lead] * alpha_prec
                else:
                    leads_sim_rescaled[lead] = leads_sim_best[lead]  # no scaling
        ...
        """ecg.plot_ecg([times_s, times_target_s],
                     [sim_leads_normed, target_leads_normed],
        ...
        print(f"{patient_id=} {ecg_fig_no=}")

        # PLOTTING OF ONE GLOBAL RESCALING FACTOR LEADS
        ecg.plot_ecg([times_s, times_target_s],
                     [leads_sim_rescaled, leads_target],
                     xlims=[0, 0.45], colors=["red", "black"], fig_no=ecg_fig_no,
                     title=target + run_id, show=False,
                     labels=["Inferred", "Target"])

        # TODO fix repol=True etc

        # Finding iter nos of where times + ECGs are stored for best params
        iter_nos_to_pop_ids = ids_to_storage_iter_nos([best_params_reg])
        ...
        if save_analysis:
            ...
            if not repol:
                print("This is what we'd save to bestqrsparams:")
                print(f"{best_x_params[0]=}")
